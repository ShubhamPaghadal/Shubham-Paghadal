            ...
            <nav className={`fixed top-0 left-0 right-0 z-100 px-4 sm:px-8 transition-all duration-500 ${scrolled || isOpen ? 'py-3 bg-background/90 backdrop-blur-xl border-b border-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.3)]' : 'py-5 sm:py-6 bg-transparent'
                ...
                <div className="max-w-[1400px] mx-auto flex items-center justify-between">
                        ...
                        {navItems.map((item) => {
                            ...
                            return (
                                <Link
                                    key={item.name}
                                    href={item.path}
                                    className={`relative group py-2 ${!isActive ? 'nav-link-dot' : ''}`}
                                >
                    ...
                    <div className="hidden xl:flex items-center gap-4">
                        <div className="flex items-center gap-2 mr-4">
                            {socialLinks.map(({ Icon, href }, idx) => (
                                <a
                                    key={idx}
                                    href={href}
                                    className="social-icon-fill w-10 h-10 flex items-center justify-center rounded-lg bg-brand-dark/50 hover:text-white text-gray-400 transition-all duration-300 transform hover:-translate-y-1"
                                >
                        ...
                        <Link href="/contact">
                            <button className="btn-shimmer btn-glow-border bg-brand-yellow text-gray-900 font-semibold px-6 py-2.5 rounded-lg hover:bg-[#ebd04f] transition-colors shadow-[0_0_20px_rgba(252,203,110,0.3)] hover:shadow-[0_0_25px_rgba(252,203,110,0.5)] transform hover:scale-105 duration-200 cursor-pointer">
                                Contact Me
                            </button>
                        </Link>
                    </div>
                                ...
                                <div className="pt-10 border-t border-white/5 flex flex-col items-center gap-10">
                                    <div className="flex items-center gap-4 max-[360px]:flex-col">
                                        {socialLinks.map(({ Icon, href }, idx) => (
                                            <motion.a
                                                ...
                                                whileTap={{ scale: 0.9 }}
                                                className="social-icon-fill w-14 h-14 flex items-center justify-center rounded-2xl bg-white/5 border border-white/5 text-gray-400 hover:text-white transition-all"
                                            >
                                                <Icon size={24} />
                                            </motion.a>
                                        ))}
                                    ...
                                    >
                                        <Link href="/contact" onClick={() => setIsOpen(false)}>
                                            <button className="btn-shimmer btn-pulse-ring w-full bg-brand-yellow text-brand-dark font-black px-8 py-5 rounded-2xl text-xl shadow-2xl active:scale-95 transition-all">
                                                Contact Me
                                            </button>
                                        </Link>
                                    </motion.div>
